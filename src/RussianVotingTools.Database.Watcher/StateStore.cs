...
namespace RussianVotingTools.Database.Watcher
{
		...
		: IDisposable
	{
		...
		public (byte[] signature, long height, long totalTransactionCount) GetLastBlockInfo()
		{
			using var session = _sessionFactory.OpenSession();
			session.DefaultReadOnly = true;
			var ret = session.Query<Block>()
			...
			var trCount = session.Query<Tx>().Count();
